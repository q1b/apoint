      ...
      <thead {...headerProps} className={cn(headerProps.className, "mb-2")}>
        <tr className="w-full">
          {weekDays.map((day, index) => (
            <th className="text-slate-400 text-lg" key={index}>
              {day}
            </th>
          ))}
        </tr>
  ...
  return (
    <td {...cellProps}>
      <div
        ...
        className={`px-1 py-1.5 rounded-full outline-none ring-1 ring-transparent focus:ring-blue-500 flex flex-col items-center justify-start ${
          isSelected
